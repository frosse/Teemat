...
public class AwayConstraint implements IConstraint, Cloneable{
            ...
            for ( int j = 0; j < series.series.get (i ).size(); j++ ) {
                if( cantPlay.containsKey(i+1)) {
                    for ( int s :cantPlay.get( i+1 ) ) {
                        if( s == series.series.get( i ).get( j ).getAway() ) {
                            teamErrors[j][s-1]++;
                        }
                    }
                }
    ...
    public void printErrors() {
        System.out.println("Away constraint");
    ...
    private void readInfoFromFile() {
        ...
        try {
            reader = new BufferedReader(new FileReader("awayConstraint.txt"));
    ...
    public Object clone() throws CloneNotSupportedException {
        AwayConstraint clone = (AwayConstraint)super.clone();
